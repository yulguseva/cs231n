...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # tanh' = 1/cosh^2 = 4/(e^x - e^-x)^2
    x, prev_h, Wx, Wh, b = cache
    
    A = x.dot(Wx) + prev_h.dot(Wh) + b
    dtanh =4/((np.exp(A)+np.exp(-A))**2)
    dtanh = dtanh*dnext_h
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)): 
        dh_full = dprev_h + dh[:,t,:]
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_full, cache[t])
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c_full = dnext_c + dtanh_next_c * (1/np.cosh(next_c)**2)
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)): 
        dh_full = dprev_h + dh[:,t,:]
        dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dh_full, dprev_c, cache[t])
